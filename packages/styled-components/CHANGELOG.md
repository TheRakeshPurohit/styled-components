...
- Plugins moved to a dedicated `styled-components/plugins` subpath, and first-party plugins ship there. ([plugins-subpath.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/plugins-subpath.md))
...
- Client styles inject through `useInsertionEffect`. ([buffered-injection-default.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/buffered-injection-default.md))
...
- `createGlobalStyle` rules land before any consumer layout effect. ([global-styles-before-layout-effects.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/global-styles-before-layout-effects.md))
...
- React Native: CSS `transition`, `@keyframes`, and `@starting-style` animate. ([native-animation.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/native-animation.md))
...
- React Native: several CSS properties are supported across platforms: `caret-color`, `object-fit`, `backface-visibility`, and `outline-offset`. ([native-caret-color-and-passthroughs.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/native-caret-color-and-passthroughs.md))
...
- React Native: the `corner-shape` property is supported, at the fidelity each value can actually be drawn. ([native-corner-shape.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/native-corner-shape.md))
...
- React Native supports CSS custom properties through the component cascade. ([native-custom-properties.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/native-custom-properties.md))
...
- React Native: CSS font properties accept the full CSS grammar and produce identical results on iOS, Android, and the web. ([native-fonts.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/native-fonts.md))
...
- React Native: relative color syntax is supported. You can write values like `oklch(from #f00 calc(l - 0.15) c h)` to derive a new color from a base color, and styled-components renders the result consistently on iOS, Android, and the web. The `oklch()`, `oklab()`, `lch()`, and `lab()` forms are supported, alongside `rgb()`, `hsl()`, `hwb()`, and `color()`. ([native-relative-color.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/native-relative-color.md))
...
- styled-components does no work in a passive or layout effect on its render path. ([no-passive-or-layout-effects.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/no-passive-or-layout-effects.md))
...
- Renaming `anchor-name` on React Native releases the old anchor. ([anchor-name-rename-releases-old-rect.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/anchor-name-rename-releases-old-rect.md))
...
- React Native: `perspective` is supported as a standalone property, so it can combine with child transforms like `rotateY` or `rotateX` to create depth. Very small values are clamped to `1px` to match browser behavior. ([native-perspective-transform-style.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/native-perspective-transform-style.md))
...
- A React Native scroller only snaps if it declared `scroll-snap-type` itself. ([scroll-snap-needs-a-declared-type.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/scroll-snap-needs-a-declared-type.md))
...
- Fix wrapping a component whose props are a union. The wrapped version accepted only the props common to every member, so a prop belonging to just one member was rejected even though the unwrapped component accepts it. The same applied when `as` pointed at such a component. ([union-props-survive-styled.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/union-props-survive-styled.md))
- Fix wrapping a component whose props cannot be inspected statically, such as the polymorphic factory components Mantine ships. Every prop was rejected, `children` included. Declaring props of your own on the wrapper no longer switches the behavior back off either. ([untyped-target-props.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/untyped-target-props.md))
- A development warning fires when server-rendered styles reach the browser inside a `<style>` tag that React manages through its `precedence` attribute. React serves such a tag under its own attributes, so styled-components cannot recognize the styles as its own and injects every rule a second time on the client. The page still looks correct, which is why this goes unnoticed, and the warning names the fix: emit the tag from `ServerStyleSheet#getStyleElement()` and pass neither `precedence` nor `href`. ([warn-unadoptable-server-styles.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/warn-unadoptable-server-styles.md))
