...
- React Native: `createTheme()` now works exactly the way it does on web. Pass the returned object to `ThemeProvider`, reference leaves in your styled components, and the current theme resolves automatically. ([createTheme-native-parity.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/createTheme-native-parity.md))
...
- Removed the `disableCSSOMInjection` prop on `<StyleSheetManager>` and the `SC_DISABLE_SPEEDY` / `REACT_APP_SC_DISABLE_SPEEDY` environment variables. Added a new `extractCSS` export. ([extract-css-and-drop-speedy-toggle.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/extract-css-and-drop-speedy-toggle.md))
...
- Mounting the same `createGlobalStyle` component multiple times now emits its CSS only once. Previously each mount produced its own copy of the stylesheet rules. Rendering output stays byte-stable across SSR, client, and Server Components. ([global-style-useid.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/global-style-useid.md))
- React Native: CSS-to-style-object translation is now built in. Several long-standing limitations go away on the native path. ([native-transform-layer.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/native-transform-layer.md))
...
- React Native: CSS `transition`, `@keyframes`, and `@starting-style` now animate. ([native-animation-adapter.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/native-animation-adapter.md))
...
- Added `box-sizing` and `hyphens` support to React Native styles. ([native-box-sizing-and-hyphens.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/native-box-sizing-and-hyphens.md))
...
- React Native: descendant and child combinator selectors now work across styled components. ([native-combinator-selectors.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/native-combinator-selectors.md))
...
- React Native now supports CSS custom properties through the component cascade. ([native-css-var-warn.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/native-css-var-warn.md))
...
- Expanded the React Native CSS surface with four polyfills that previously dropped silently or warned: ([native-font-line-height-caret-line-width.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/native-font-line-height-caret-line-width.md))
...
- React Native: `:has(<simple>)` selector now works. ([native-has-selector.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/native-has-selector.md))
...
- React Native now respects CSS `!important`. ([native-important-support.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/native-important-support.md))
...
- CSS `overscroll-behavior` and `scrollbar-width` now work on React Native. Apply them to a `styled.ScrollView`, `styled.FlatList`, `styled.SectionList`, or `styled.VirtualizedList`: ([native-scroll-surfaces.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/native-scroll-surfaces.md))
...
- The arity-2 `.attrs((props, ast) => ...)` callback now sees `ast` as a non-optional `CompiledAst`, so authors no longer need to optional-chain to satisfy the TypeScript compiler under `strict: true`. The arity-1 form (`.attrs((props) => ...)`) is unchanged. ([attrs-ast-required-in-arity-2.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/attrs-ast-required-in-arity-2.md))
...
- React Native: `lab()` and `lch()` now produce correct colors when channels are written as percentages. `lab(50% 0 0)` resolves to mid-gray as expected, where it previously produced near-black. Per CSS Color L4, each space has its own range: ([color-percent-channels.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/color-percent-channels.md))
...
- Empty CSS custom property values are now preserved. ([empty-custom-property-values.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/empty-custom-property-values.md))
...
- Fixed selector resolution for nested rules whose parent contains a comma inside `:is()`, `:where()`, `:has()`, or an attribute selector. ([is-where-cross-product-bug.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/is-where-cross-product-bug.md))
...
- `object-fit` on a styled Image now applies correctly across iOS, Android, and the web. Previously the value reached the underlying Image differently on each target, leaving the web bar without the requested fit. ([native-object-fit-rnweb.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/native-object-fit-rnweb.md))
- `perspective` now works as a standalone property on React Native, so it can be combined with child transforms like `rotateY` or `rotateX` to create depth. Very small values are clamped to `1px` to match browser behavior. `transform-style: preserve-3d` shows a one-time development warning on iOS and Android because React Native does not expose that behavior yet; `react-native-web` continues to let the browser handle both properties. ([native-perspective-transform-style.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/native-perspective-transform-style.md))
- Fixes React Native spec-compliance edge cases in the `place-content` shorthand. `start`, `end`, and `space-evenly` now match CSS Box Alignment behavior. ([native-place-content.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/native-place-content.md))
...
- `rem` length values now work on React Native. They use the app's root font size, `16` by default, so `width: 1rem` becomes `16` and `width: 2rem` becomes `32`. `rem` works on its own and inside `calc()`. `react-native-web` continues to let the browser handle it. ([native-rem-unit.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/native-rem-unit.md))
...
- `transform-box` now shows a one-time development warning on iOS and Android explaining that React Native transforms use the view center as their reference box. Use `transform-origin` when you need to move the pivot point. `react-native-web` continues to let the browser handle `transform-box`. ([native-transform-box-warn.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/native-transform-box-warn.md))
- Development builds now warn when `vertical-align` is set on a native `<Text>` or `<TextInput>` running on iOS. ([native-vertical-align-ios-warning.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/native-vertical-align-ios-warning.md))
...
- Fixed a styling leak when a nested `<StyleSheetManager>` sits beside other children of an outer `<StyleSheetManager>` in a server component tree. ([rsc-nested-stylesheetmanager.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/rsc-nested-stylesheetmanager.md))
...
- Fixed plain objects with a custom `toString()` being expanded as CSS-property blocks instead of stringified. Useful for design-token shapes where the token resolves to a default value but also carries alternate sub-values: ([toString-design-tokens.md](https://github.com/TheRakeshPurohit/styled-components/commits/main/.changeset/toString-design-tokens.md))
